...
def test_mlx_example_rejects_missing_counters_but_accepts_zero_gpu_power(mocker, capsys, missing, country):
    ...
    arguments = ["measure_mlx.py", "--repeats", "1"]
    if country:
        arguments.extend(["--country", country])
    mocker.patch.object(sys, "argv", arguments)
    detect_country = mocker.patch.object(Country, "get_current_country", return_value="FR")
    ...
    if missing is not None:
        ...
        assert result["co2g"] == pytest.approx(result["energy_wh"] * 74 / 1000)
    assert detect_country.call_count == (0 if country else 1)
